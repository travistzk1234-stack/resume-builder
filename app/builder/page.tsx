...
function Spinner({ light }: { light?: boolean }) {
  return (
    <span style={{
      ...
      borderTopColor: light ? "#fff" : C.text,
        ...
        <div key={s} style={{ display: "flex", alignItems: "center", flex: i < STEPS.length - 1 ? 1 : "none" as any }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            <div style={{
              ...
              background: i <= current ? C.text : C.bgSecondary,
              color: i <= current ? "#fff" : C.textTertiary,
              border: `1px solid ${i <= current ? C.text : C.border}`,
            }}>
              {i < current ? "✓" : i + 1}
            </div>
    ...
    <div style={{ position: "relative" }}>
      ...
      <input
        ...
        style={{
          ...
          fontSize: 14, border: `1px solid ${C.border}`, borderRadius: 10,
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
        ...
        {onImprove && (
          <button
            ...
            style={{
              ...
              cursor: improving || !value.trim() ? "not-allowed" : "pointer",
              background: "none", border: `1px solid ${C.border}`,
              color: C.textSecondary, display: "inline-flex", alignItems: "center",
              gap: 4, opacity: !value.trim() ? 0.4 : 1, fontFamily: "inherit", fontWeight: 500,
            }}>
      ...
      <textarea
        ...
        style={{
          ...
          minHeight: minH, lineHeight: 1.7, fontFamily: "inherit",
        }}
        onFocus={(e) => { e.target.style.borderColor = C.borderMid; e.target.style.boxShadow = "0 0 0 3px rgba(0,0,0,0.04)"; }}
        onBlur={(e) => { e.target.style.borderColor = C.border; e.target.style.boxShadow = "none"; }}
      />
...
function Btn({ children, onClick, variant = "secondary", disabled, style, size = "md" }: any) {
  ...
  const base: any = {
    ...
    gap: 6, border: "none", opacity: disabled ? 0.5 : 1,
    fontFamily: "inherit", letterSpacing: "-0.1px",
  };
  const primary: any = { background: C.text, color: "#fff" };
  const secondary: any = { background: C.bgSecondary, border: `1px solid ${C.border}`, color: C.text };
  const chosen = variant === "primary" ? primary : secondary;
  return (
    <button onClick={disabled ? undefined : onClick} style={{ ...base, ...chosen, ...style }}>
      {children}
    </button>
  );
}
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
      <div
        ...
        style={{
          ...
          border: `2px dashed ${photo ? C.borderMid : C.border}`,
          background: photo ? "transparent" : C.bgSecondary,
          cursor: "pointer", overflow: "hidden",
          display: "flex", alignItems: "center", justifyContent: "center",
        }}>
        {photo
          ? <img src={photo} style={{ width: "100%", height: "100%", objectFit: "cover" as any }} />
...
function ResumePreview({ form, resume, template, photo }: any) {
  ...
  return (
    <div id="resume-preview" style={{
      ...
      fontFamily: "Georgia,serif", color: "#1a1a1a", fontSize: 13,
      lineHeight: 1.65, border: `1px solid ${C.border}`, boxSizing: "border-box" as any,
    }}>
      ...
      {resume?.experience?.length > 0 && (
        <ResumeSection accent={accent} title="Experience">
          {resume.experience.map((exp: any, i: number) => (
            <div key={i} style={{ marginBottom: i < resume.experience.length - 1 ? 14 : 0 }}>
              ...
              <ul style={{ margin: 0, paddingLeft: 18 }}>
                {exp.bullets?.map((b: string, j: number) => <li key={j} style={{ marginBottom: 3, color: "#444" }}>{b}</li>)}
              </ul>
            </div>
          ))}
        </ResumeSection>
      ...
      {resume?.education?.length > 0 && (
        <ResumeSection accent={accent} title="Education">
          {resume.education.map((ed: any, i: number) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <div>
                <strong style={{ fontFamily: "sans-serif", fontSize: 13 }}>{ed.degree}</strong>
                <p style={{ margin: "2px 0 0", fontSize: 12, color: "#777", fontFamily: "sans-serif" }}>{ed.institution}</p>
              </div>
      ...
      {resume?.skills?.length > 0 && (
        <ResumeSection accent={accent} title="Skills">
          <p style={{ margin: 0, fontFamily: "sans-serif", color: "#444", lineHeight: 1.8 }}>{resume.skills.join(" · ")}</p>
        </ResumeSection>
      )}

      {resume?.achievements?.length > 0 && (
        <ResumeSection accent={accent} title="Achievements">
          <ul style={{ margin: 0, paddingLeft: 18 }}>
            {resume.achievements.map((a: string, i: number) => <li key={i} style={{ marginBottom: 3, color: "#444", fontFamily: "sans-serif" }}>{a}</li>)}
          </ul>
        </ResumeSection>
      )}
    </div>
    ...
    <div style={{ background: C.bgSecondary, borderRadius: 10, padding: "16px 20px", marginTop: 14, border: `1px solid ${C.border}` }}>
      ...
      {ats.keywords?.length > 0 && (
        <div style={{ marginBottom: 10 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap" as any, gap: 4 }}>
            {ats.keywords.map((k: string) => <span key={k} style={{ background: C.successBg, color: C.success, borderRadius: 999, fontSize: 11, padding: "3px 10px", fontWeight: 500 }}>{k}</span>)}
          </div>
        </div>
      )}
      {ats.missing?.length > 0 && (
        <div style={{ marginBottom: 10 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap" as any, gap: 4 }}>
            {ats.missing.map((k: string) => <span key={k} style={{ background: C.dangerBg, color: C.danger, borderRadius: 999, fontSize: 11, padding: "3px 10px", fontWeight: 500 }}>{k}</span>)}
          </div>
        </div>
      )}
      {ats.tips?.length > 0 && (
        <div>
          ...
          {ats.tips.map((tip: string, i: number) => (
            <p key={i} style={{ fontSize: 12, color: C.textSecondary, margin: "0 0 5px", paddingLeft: 10, borderLeft: `2px solid ${C.border}`, lineHeight: 1.5 }}>{tip}</p>
          ))}
        </div>
      )}
    </div>
...
export default function App() {
  ...
  const improveField = async (id: string) => {
    ...
    try {
      ...
      setError("Improve failed. Try again.");
    }
  ...
  const buildResume = async () => {
    ...
    try {
      ...
      setError("Failed to build resume. Please try again.");
    }
  ...
  const genLinkedIn = async () => {
    ...
    try {
      ...
      setError("LinkedIn generation failed.");
    }
  ...
  return (
    <div style={{ fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", color: C.text, background: C.bgTertiary, minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "2rem 1.5rem 4rem" }}>
        ...
        {error && (
          <div style={{ background: C.dangerBg, border: "1px solid rgba(185,28,28,0.2)", borderRadius: 10, padding: "12px 16px", fontSize: 13, color: C.danger, marginBottom: 16 }}>
            {error}
          </div>
        )}
        ...
        {step === 1 && (
          <div>
            ...
            <SectionCard title="Work history" subtitle="Describe your roles, responsibilities, and impact">
              <TextArea
                label="Work experience" id="experience" value={form.experience} onChange={update} minH={150}
                hint="Write casually — AI will improve it"
                placeholder="e.g. I worked at Google for 3 years doing backend engineering. Led a team of 5 people and built APIs that handled lots of traffic."
                onImprove={() => improveField("experience")} improving={improvingField === "experience"}
              />
            </SectionCard>

            <SectionCard title="Achievements & extras" subtitle="Awards, projects, publications, volunteering">
              <TextArea
                label="Achievements" id="achievements" value={form.achievements} onChange={update} minH={90}
                placeholder="e.g. Won a company hackathon, published an article on Medium, open-source project with 2k GitHub stars"
        ...
        {step === 2 && (
          <div>
            ...
            <SectionCard title="Education" subtitle="Degrees, certifications, courses">
              <TextArea
                label="Education background" id="education" value={form.education} onChange={update} minH={90}
            ...
            <SectionCard title="Skills" subtitle="Technical skills, tools, languages, soft skills">
              <TextArea
                label="Your skills" id="skills" value={form.skills} onChange={update} minH={80}
                placeholder="e.g. React, TypeScript, Python, Node.js, Figma, AWS, Docker, project management"
              ...
              <div style={{ display: "flex", gap: 12 }}>
                {TEMPLATES.map(t => (
                  <button
                    ...
                    style={{
                      padding: "12px 22px", borderRadius: 10, cursor: "pointer", fontWeight: 500, fontSize: 13,
                      border: template === t.id ? `2px solid ${t.accent}` : `1px solid ${C.border}`,
                      background: template === t.id ? "#fff" : C.bgSecondary,
                      color: template === t.id ? t.accent : C.textSecondary,
          ...
          <div>
            <div style={{ marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {TEMPLATES.map(t => (
                  <button
                    key={t.id} onClick={() => setTemplate(t.id)} title={t.label}
                    style={{ width: 22, height: 22, borderRadius: "50%", background: t.accent, border: template === t.id ? "3px solid #1a1a1a" : "2px solid transparent", cursor: "pointer", padding: 0 }}
                  />
                ))}
              </div>
            </div>
